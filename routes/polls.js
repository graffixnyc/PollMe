...
router.post("/createpoll", function(request, response) {
    ...
    if (request.isAuthenticated()) {
        usersPollsData.addPoll(newPoll.category, now.toDateString(), newPoll.question, newPoll.choice1, newPoll.choice2, newPoll.choice3, newPoll.choice4, request.user._id).then((pollid) => {
